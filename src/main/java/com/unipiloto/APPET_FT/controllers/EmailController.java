...
public class EmailController {
    ...
    @PostMapping("/mensaje-recorrido/{idMascota}/{correo}")
    public ResponseEntity<Object> enviarRecorrido(@RequestBody CorreoDTO correoDTO, @PathVariable Integer idMascota, @PathVariable String correo){
        Map<String, String> response = new HashMap<>();
        ...
        correoDTO.setMensaje("Le escribe " + nombreVeterinario + ", su cuidador de Mascotas\n" + correoDTO.getMensaje());
        response.put("mensaje", "Correo enviado exitosamente.");
        emailService.enviarCorreo(correoPropietario, correoDTO.getAsunto(), correoDTO.getMensaje());
        return ResponseEntity.ok(response);
    }
